...
/**
 * ERC721 Default Escrow ObligationData type
 */
export type Erc721DefaultEscrowObligationData = {
  token: `0x${string}`;
  tokenId: bigint;
  arbiter: `0x${string}`;
  demand: `0x${string}`;
...
/**
 * Encodes ERC721DefaultEscrowObligation.ObligationData to bytes.
 * @param data - struct ObligationData {address token, uint256 tokenId, address arbiter, bytes demand}
 * @returns abi encoded bytes
 */
export const encodeObligation = (data: Erc721DefaultEscrowObligationData): `0x${string}` => {
  return encodeAbiParameters([erc721EscrowObligationDataType], [data]);
};

/**
 * Decodes ERC721DefaultEscrowObligation.ObligationData from bytes.
 * @param obligationData - ObligationData as abi encoded bytes
 * @returns the decoded ObligationData object
 */
export const decodeObligation = (obligationData: `0x${string}`): Erc721DefaultEscrowObligationData => {
  return decodeAbiParameters(
    [erc721EscrowObligationDataType],
    obligationData,
  )[0] as Erc721DefaultEscrowObligationData;
};

export type Erc721DefaultEscrowClient = ReturnType<typeof makeErc721DefaultEscrowClient>;

export const makeErc721DefaultEscrowClient = (viemClient: ViemClient, addresses: Erc721Addresses) => {
