...
/**
 * ERC1155 Default Escrow ObligationData type
 */
export type Erc1155DefaultEscrowObligationData = {
  token: `0x${string}`;
  tokenId: bigint;
  amount: bigint;
  arbiter: `0x${string}`;
...
/**
 * Encodes ERC1155DefaultEscrowObligation.ObligationData to bytes.
 * @param data - struct ObligationData {address token, uint256 tokenId, uint256 amount, address arbiter, bytes demand}
 * @returns abi encoded bytes
 */
export const encodeObligation = (data: Erc1155DefaultEscrowObligationData): `0x${string}` => {
  return encodeAbiParameters([erc1155EscrowObligationDataType], [data]);
};

/**
 * Decodes ERC1155DefaultEscrowObligation.ObligationData from bytes.
 * @param obligationData - ObligationData as abi encoded bytes
 * @returns the decoded ObligationData object
 */
export const decodeObligation = (obligationData: `0x${string}`): Erc1155DefaultEscrowObligationData => {
  return decodeAbiParameters(
    [erc1155EscrowObligationDataType],
    obligationData,
  )[0] as Erc1155DefaultEscrowObligationData;
};

export type Erc1155DefaultEscrowClient = ReturnType<typeof makeErc1155DefaultEscrowClient>;

export const makeErc1155DefaultEscrowClient = (viemClient: ViemClient, addresses: Erc1155Addresses) => {
