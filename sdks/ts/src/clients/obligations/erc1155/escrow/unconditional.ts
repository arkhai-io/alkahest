...
import { abi as erc1155EscrowAbi } from "../../../../contracts/obligations/escrow/unconditional/UnconditionalERC1155EscrowObligation";
...
/**
 * ERC1155 Unconditional Escrow ObligationData type
 */
export type Erc1155UnconditionalEscrowObligationData = {
  token: `0x${string}`;
  tokenId: bigint;
  amount: bigint;
  arbiter: `0x${string}`;
...
/**
 * Encodes ERC1155UnconditionalEscrowObligation.ObligationData to bytes.
 * @param data - struct ObligationData {address token, uint256 tokenId, uint256 amount, address arbiter, bytes demand}
 * @returns abi encoded bytes
 */
export const encodeObligation = (data: Erc1155UnconditionalEscrowObligationData): `0x${string}` => {
  return encodeAbiParameters([erc1155EscrowObligationDataType], [data]);
};

/**
 * Decodes ERC1155UnconditionalEscrowObligation.ObligationData from bytes.
 * @param obligationData - ObligationData as abi encoded bytes
 * @returns the decoded ObligationData object
 */
export const decodeObligation = (obligationData: `0x${string}`): Erc1155UnconditionalEscrowObligationData => {
  return decodeAbiParameters(
    [erc1155EscrowObligationDataType],
    obligationData,
  )[0] as Erc1155UnconditionalEscrowObligationData;
};

export type Erc1155UnconditionalEscrowClient = ReturnType<typeof makeErc1155UnconditionalEscrowClient>;

export const makeErc1155UnconditionalEscrowClient = (viemClient: ViemClient, addresses: Erc1155Addresses) => {
