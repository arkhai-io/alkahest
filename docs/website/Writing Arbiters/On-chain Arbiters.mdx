...
2. **Implement `check`**: The main validation function that returns a boolean
...
Asynchronous arbiters handle validation that cannot be completed in a single transaction. They maintain internal state across multiple transactions and only return true from `check` once a multi-step process has been completed.
...
4. **Deferred Validation**: `check` returns false until the process completes, then returns the final decision
