...
There are utility contracts that provide a convenient interface for settling token trades atomically, and SDKs in TypeScript, Rust, and Python that wrap these. Create escrows through the token escrow clients or contracts, then use BarterUtils to fulfill compatible escrows in one transaction.

The functions to fulfill any type of escrow demanding a token type (`payXforY`) are available in \[TokenType]BarterUtils.sol, or the corresponding barter module of each SDK (e.g. `client.erc20.barter`, `client.tokenBundle.barter`...). Available token types are native tokens (ETH), ERC-20, ERC-721, ERC-1155, and bundles of all of these together.
...
bytes32 paymentUid = erc721BarterUtils.payErc721AndCollect(escrowUid);
...
const payment = await bobClient.erc721.barter.payErc721AndCollect(attested.uid);
...
payment = await bob_client.erc721.barter.pay_erc721_and_collect(escrow["log"]["uid"])
...
payment = await bob_client.erc20.barter.permit_and_pay_erc20_and_collect(escrow["log"]["uid"])
