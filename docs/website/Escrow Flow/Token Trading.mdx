...
<TabItem value="alloy" label="Alloy">
...
The CLI's `barter create` command is a convenience path for token-for-token offers. For manual escrow creation, use `escrow create`.
...
Or use the CLI barter commands for simple token-for-token trades:
...
erc20EscrowObligation.collect(escrowUid, paymentUid);
```
...
escrow_contract
    .collect(escrow_uid, payment_uid)
    .send()
    .await?
    .get_receipt()
    .await?;
...
There are utility contracts that provide a convenient interface for settling token trades atomically, and SDKs in TypeScript and Rust wrap these. Create escrows through the token escrow clients or contracts, then use `AtomicPaymentUtils` to fulfill compatible escrows in one transaction.

The functions to pay with a token type and collect the escrow (`payErc20AndCollect`, `payErc721AndCollect`, etc.) are available on `AtomicPaymentUtils`, and the SDKs expose them from each token's payment client (e.g. `client.erc20.payment`, `client.tokenBundle.payment`). Available payment types are native tokens (ETH), ERC-20, ERC-721, ERC-1155, and bundles of all of these together.
...
```solidity
// Alice: Create escrow offering ERC-20 for ERC-721 through the escrow contract
IERC20(usdcToken).approve(address(erc20Escrow), 1000e6);
bytes32 escrowUid = erc20Escrow.doObligation(
    ERC20EscrowObligation.ObligationData({
        token: usdcToken,
        amount: 1000e6,
        arbiter: address(erc721Payment),
        demand: abi.encode(erc721Token, uint256(42), alice)
    }),
    expiration
);

// Bob: Fulfill escrow by paying ERC-721 and collecting the ERC-20 escrow
IERC721(erc721Token).approve(address(atomicPaymentUtils), 42);
bytes32 paymentUid = atomicPaymentUtils.payErc721AndCollect(escrowUid);

// ERC-20 permit settlement remains available when the fulfiller pays with ERC-20.
```
...
const { hash, attested } = await aliceClient.erc20.escrow.default.approveAndCreate(
  { address: usdcToken, value: 1000000000n },
  { arbiter: erc721PaymentAddress, demand: encodedErc721PaymentDemand },
  0n,
);

// Bob: Approve and fulfill escrow by paying ERC-721 and collecting the ERC-20 escrow
await bobClient.erc721.util.approve({ address: erc721Token, id: 42n }, "atomicPayment");
const payment = await bobClient.erc721.payment.payErc721AndCollect(attested.uid);

// Can also use direct escrow/payment for custom demands (approveAndCreate combines both steps)
const escrow = await aliceClient.erc20.escrow.default.approveAndCreate(
  { address: usdcToken, value: 1000000000n },
...
let escrow_receipt = alice_client
    ...
    .escrow()
    .default()
    .approve_and_create(&bid, &demand, 0)
    .await?;
let escrow_uid = DefaultAlkahestClient::get_attested_event(escrow_receipt)?.uid;

// Bob: Approve and fulfill escrow by paying ERC-721 and collecting the ERC-20 escrow
bob_client.erc721().approve(&ask, ApprovalPurpose::AtomicPayment).await?;
let payment_receipt = bob_client
    .erc721()
    .payment()
    .pay_erc721_and_collect(escrow_uid)
    .await?;
...
escrow = await alice_client.erc20.escrow.default.approve_and_create(bid, demand, 0)

# Bob: Approve and fulfill escrow by paying ERC-721 and collecting the ERC-20 escrow
await bob_client.erc721.util.approve(ask, "atomic_payment")
payment = await bob_client.erc721.payment.pay_erc721_and_collect(escrow["log"]["uid"])
...
payment = await bob_client.erc20.payment.permit_and_pay_erc20_and_collect(escrow["log"]["uid"])

# Can also use direct escrow/payment for custom demands (approve_and_create combines both steps)
escrow = await alice_client.erc20.escrow.default.approve_and_create(
...
The CLI exposes token-trade convenience commands. Use `--approve` to approve the escrow or atomic payment utility, or use `--permit` when fulfilling an ERC20 ask:

```bash
# Alice: Create barter (ERC20 for ERC20) with approval
alkahest --private-key 0xALICE_KEY barter create \
  --bid-type erc20 --ask-type erc20 \
  --bid-token 0xUSDC --bid-amount 1000000000 \
  --ask-token 0xEURC --ask-amount 900000000 \
