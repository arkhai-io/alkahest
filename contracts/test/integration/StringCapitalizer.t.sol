...
contract StringCapitalizerTest is Test {
    ...
    function testCheckObligationWithValidCapitalization() public {
        ...
        StringCapitalizer.DemandData memory demand = StringCapitalizer.DemandData({query: "hello world"});

        // Create an obligation with the capitalized result
        StringObligation.ObligationData memory obligationData =
            StringObligation.ObligationData({item: "HELLO WORLD", schema: bytes32(0)});

        // Create the attestation as bob
        vm.prank(bob);
        bytes32 obligationUID = stringObligation.doObligation(obligationData, bytes32(0));
    ...
    function testCheckObligationWithInvalidCapitalization() public {
        ...
        StringObligation.ObligationData memory obligationData =
            StringObligation.ObligationData({item: "Hello World", schema: bytes32(0)}); // Only first letters capitalized
    ...
    function testCheckObligationWithMixedCase() public {
        ...
        StringCapitalizer.DemandData memory demand = StringCapitalizer.DemandData({query: "HeLLo WoRLd 123!"});

        // Create an obligation with correctly capitalized result
        StringObligation.ObligationData memory obligationData =
            StringObligation.ObligationData({item: "HELLO WORLD 123!", schema: bytes32(0)}); // All letters uppercase, others unchanged
    ...
    function testCheckObligationWithNumbers() public {
        ...
        StringCapitalizer.DemandData memory demand = StringCapitalizer.DemandData({query: "test123@email.com"});

        // Create an obligation with correctly capitalized result
        StringObligation.ObligationData memory obligationData =
            StringObligation.ObligationData({item: "TEST123@EMAIL.COM", schema: bytes32(0)});
    ...
    function testCheckObligationWithWrongLength() public {
        // Create a demand
        StringCapitalizer.DemandData memory demand = StringCapitalizer.DemandData({query: "hello"});

        // Create an obligation with different length string
        StringObligation.ObligationData memory obligationData =
            StringObligation.ObligationData({item: "HELLO WORLD", schema: bytes32(0)}); // Different length
    ...
    function testCheckObligationWithFulfilling() public {
        ...
        bytes32 escrowUID = stringObligation.doObligation(
            StringObligation.ObligationData({item: "request", schema: bytes32(0)}), bytes32(0)
        );
        ...
        vm.prank(bob);
        bytes32 obligationUID = stringObligation.doObligation(obligationData, escrowUID);
    ...
    function testFuzzCapitalization(string memory input, string memory output) public {
        ...
        StringCapitalizer.DemandData memory demand = StringCapitalizer.DemandData({query: input});

        StringObligation.ObligationData memory obligationData =
            StringObligation.ObligationData({item: output, schema: bytes32(0)});
        ...
        assertEq(result, expectedResult, "Arbiter result should match expected capitalization check");
    }

    function isProperlyCapitalized(string memory input, string memory output) internal pure returns (bool) {
