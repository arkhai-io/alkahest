...
import {TokenBundleBarterUtils} from "@src/utils/barter/TokenBundleBarterUtils.sol";
...
contract TokenBundleBarterUtilsTest is Test, IERC1155Receiver {
    ...
    function _setupContracts() internal {
        ...
        barterUtils = new TokenBundleBarterUtils(eas, escrowObligation, paymentObligation);
    }
    ...
    function _createBidBundle() internal pure returns (TokenBundleEscrowObligation.ObligationData memory) {
    ...
    function _createAskBundle() internal pure returns (TokenBundlePaymentObligation.ObligationData memory) {
        address[] memory erc20Tokens = new address[](1);
        uint256[] memory erc20Amounts = new uint256[](1);
        address[] memory erc721Tokens = new address[](1);
        uint256[] memory erc721TokenIds = new uint256[](1);
    ...
    function _getERC20PermitSignature(
        ...
        return TokenBundleBarterUtils.ERC20PermitSignature({v: v, r: r, s: s, deadline: deadline});
    }
    ...
    function testBundleTradeWithManualApprovals() public {
        ...
        assertEq(
            askErc1155TokenA.balanceOf(address(escrowObligation), bidBundle.erc1155TokenIds[0]),
        ...
        vm.startPrank(bob);
        erc20TokenB.approve(address(barterUtils), askBundle.erc20Amounts[0]);
        askErc721TokenB.approve(address(barterUtils), askBundle.erc721TokenIds[0]);
        askErc1155TokenB.setApprovalForAll(address(barterUtils), true);

        bytes32 sellAttestation = barterUtils.payBundleForBundle(buyAttestation);
        vm.stopPrank();
        ...
        // ERC20
        assertEq(erc20TokenA.balanceOf(bob), bidBundle.erc20Amounts[0], "Bob didn't receive correct Token A amount");
        assertEq(erc20TokenB.balanceOf(alice), askBundle.erc20Amounts[0], "Alice didn't receive correct Token B amount");
        assertEq(
        ...
        // ERC721
        assertEq(askErc721TokenA.ownerOf(bidBundle.erc721TokenIds[0]), bob, "ERC721 A not transferred to Bob");
        assertEq(askErc721TokenB.ownerOf(askBundle.erc721TokenIds[0]), alice, "ERC721 B not transferred to Alice");
        ...
        assertEq(
            erc20TokenA.balanceOf(address(escrowObligation)), 0, "Escrow contract should have no remaining Token A"
        );
        assertEq(
            askErc1155TokenA.balanceOf(address(barterUtils), bidBundle.erc1155TokenIds[0]),
            0,
    ...
    function testBundleTradeWithPermits() public {
        ...
        // Setup Bob's permits
        TokenBundleBarterUtils.ERC20PermitSignature[] memory bobPermits =
            new TokenBundleBarterUtils.ERC20PermitSignature[](1);
        bobPermits[0] = _getERC20PermitSignature(
            erc20TokenB, BOB_PRIVATE_KEY, address(barterUtils), askBundle.erc20Amounts[0], block.timestamp + 1
        );
        ...
        bytes32 sellAttestation = barterUtils.permitAndPayBundleForBundle(buyAttestation, bobPermits);
        vm.stopPrank();
    ...
    function _verifyFinalState() internal view {
        ...
        assertEq(erc20TokenA.balanceOf(alice), 900 * 10 ** 18, "Alice's Token A balance incorrect");
        assertEq(erc20TokenA.balanceOf(bob), 100 * 10 ** 18, "Bob's Token A balance incorrect");
        assertEq(erc20TokenB.balanceOf(alice), 200 * 10 ** 18, "Alice's Token B balance incorrect");
        assertEq(erc20TokenB.balanceOf(bob), 800 * 10 ** 18, "Bob's Token B balance incorrect");
        ...
        assertEq(askErc1155TokenA.balanceOf(bob, 1), 10, "Bob's MultiToken A balance incorrect");
        assertEq(askErc1155TokenB.balanceOf(alice, 1), 20, "Alice's MultiToken B balance incorrect");
    }
