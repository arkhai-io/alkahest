...
contract ERC8004Deployer is Test {
    string constant ARTIFACTS_DIR = "lib/erc-8004-contracts/artifacts/contracts/";
    ...
    function deployUpgradeable(string memory contractName, bytes memory v2InitData) public returns (address proxy) {
        // Step 1: Deploy the MinimalUUPS implementation (v1)
        bytes memory minimalBytecode = _readBytecode("HardhatMinimalUUPS", "HardhatMinimalUUPS");
        address minimalImpl;
        assembly {
            minimalImpl := create(0, add(minimalBytecode, 0x20), mload(minimalBytecode))
        }
        ...
        bytes memory proxyBytecode = _readBytecode("ERC1967Proxy", "ERC1967Proxy");
        bytes memory v1InitData = abi.encodeWithSignature("initialize(address)", address(0));
        bytes memory proxyCreationCode = abi.encodePacked(proxyBytecode, abi.encode(minimalImpl, v1InitData));
        assembly {
            proxy := create(0, add(proxyCreationCode, 0x20), mload(proxyCreationCode))
        }
        ...
        address realImpl;
        assembly {
            realImpl := create(0, add(realBytecode, 0x20), mload(realBytecode))
        }
    ...
    function _readBytecode(string memory fileName, string memory contractName) internal view returns (bytes memory) {
        string memory path = string.concat(ARTIFACTS_DIR, fileName, ".sol/", contractName, ".json");
