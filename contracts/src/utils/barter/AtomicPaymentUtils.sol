...
contract AtomicPaymentUtils is IERC1155Receiver {
    ...
    function _payErc20(ERC20PaymentObligation.ObligationData memory data, bytes32 refUID)
        internal
        returns (bytes32)
    {
    ...
    function _collect(address escrowContract, bytes32 escrowUid, bytes32 fulfillmentUid) internal {
        (bool success,) = escrowContract.call(abi.encodeWithSignature("collect(bytes32,bytes32)", escrowUid, fulfillmentUid));
    ...
    function _permitBundle(
        ...
        if (permits.length != data.erc20Tokens.length) revert InvalidSignatureLength();
